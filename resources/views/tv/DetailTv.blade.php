    ...
    <main>
        <div class="position-relative overflow-hidden" 
            ...
            <div class="font-zindex3 col-11">
                ...
                <h1 class="display-4 fw-normal text-uppercase">{{ $detail['name']}}</h1>
                <p class="fw-normal">
        ...
        <div class="album py-3" style="background-color: black">
            <div class="container">
                <div class="row">
                    <h4 style="color: white" class="pb-2 col-6">Episode</h4>
                    <div class="col-6">
                        <div class="dropdown d-flex">
                            <button id="seasonDropdown" class="btn btn-secondary ms-auto" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                               Season
                            </button>
                            <ul class="dropdown-menu">
                                @foreach ($seasons as $season)
                                    <li>
                                        <a href="{{ route('tv.show', ['id' => $detail['id'], 'seasonNumber' => $season['season_number']]) }}" class="dropdown-item season-item" data-season="{{ $season['season_number'] }}">
                                            {{ $season['name'] }}
                                        </a>
                                    </li>
                                @endforeach
                        ...
                        @foreach ($episode as $episode)
                            @if ($episode['season_number'] == $seasonNumber)
                                ...
                                <a class="d-flex flex-column flex-lg-row gap-3 align-items-start align-items-lg-center py-3 link-body-emphasis text-decoration-none border-top">
                                    <img src="{{ 'https://image.tmdb.org/t/p/w500'. $episode['still_path'] }}" class="img-fluid col-md-3" alt="gambar">
                                    <div class="col-lg-8">
                                    <div class="row">
                                        <p class="col-6" style="color: white">{{ $episode['name'] }}</p>
                                        <p class="col-6 text-end" style="color: white">{{ $episode['runtime'] }} Miniutes</p>
                                    </div>
        ...
        @if ($similiartv)
            <div class="album py-3" style="background-color: black">
                <div class="container">
                <h4 style="color: white" class="pb-2">Similiar Tv Searies For You</h4>
                <div class="wrapper-TopRatedMovie">
                    @foreach ($similiartv as $similiartv)
                    <div class="item movie-card-TopRatedMovie">
                    <a href="{{ route('tv.show', ['id' => $similiartv['id'], 'seasonNumber' => '1']) }}" class="non-text-decoration">
                        <img src="{{ 'https://image.tmdb.org/t/p/w500'. $similiartv['backdrop_path'] }}" class="img-fluid" alt="gambar">
                        <div class="py-2 movie-details-TopRatedMovie">
                        <p class="daleman-p-poster-TopRatedMovie col-3">
                            <i class="fa-solid fa-star" style="color: #d7a904;"></i> {{$similiartv['vote_average']}}
                        </p>
                        <div class="row">
                            <p class="col-12" style="margin-top:15%; margin-left:5%; font-weight: bold;">
                            {{ $similiartv['name']}} 
  ...
  {{-- <script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Fungsi untuk memperbarui tombol berdasarkan nilai dari URL
            function updateSeasonFromUrl() {
                const urlParams = new URLSearchParams(window.location.search);
                const seasonNumberFromUrl = urlParams.get('seasonNumber');
                if (seasonNumberFromUrl) {
                    seasonDropdown.innerHTML = 'Season ' + seasonNumberFromUrl;
                }
            }

            // Panggil fungsi untuk memperbarui tombol berdasarkan URL saat halaman dimuat
            updateSeasonFromUrl();
        });
    </script>
